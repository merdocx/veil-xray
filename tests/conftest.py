...
from api.database import Base, get_db, init_db, Key, TrafficStats  # Импортируем модели для регистрации в Base
...
def test_db(monkeypatch):
    ...
    assert get_db in app.dependency_overrides, f"get_db override не установлен! Overrides: {list(app.dependency_overrides.keys())}"
    ...
    try:
        ...
        assert str(test_db_engine.url) == test_db_url, f"Engine URL не совпадает! Got: {test_db_engine.url}, Expected: {test_db_url}"
    finally:
...
def client(test_db, mock_xray_client, disable_startup_event):
    ...
    assert get_db in app.dependency_overrides, f"get_db override не применен! Overrides: {list(app.dependency_overrides.keys())}"
    ...
    assert get_db in app.dependency_overrides, f"get_db override потерян после создания TestClient! Overrides: {list(app.dependency_overrides.keys())}"

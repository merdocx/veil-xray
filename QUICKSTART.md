# Быстрый старт Veil Xray API

## ✅ Выполненные шаги

1. ✓ Ключи Reality сгенерированы
2. ✓ Файл `.env` создан и настроен
3. ✓ База данных инициализирована
4. ✓ Зависимости установлены в виртуальное окружение
5. ✓ API сервер проверен на работоспособность

## 📋 Текущее состояние

- **Reality Public Key**: `UTJOOyjxZ/QM35IdmIWAwLF676ObaHITUvIUUKA7TQE=`
- **Reality Private Key**: `ME+tyfHhUD6tQoPT11MBy0leLM0Wu4IVg5MU7giVoW4=` (хранится в `/root/config/reality_keys.txt`)
- **API Secret Key**: Сгенерирован случайный ключ в `.env`
- **База данных**: `/root/database/veil_xray.db` (создана)

## 🚀 Запуск API сервера

### Вариант 1: Запуск вручную

```bash
cd /root
source venv/bin/activate
python3 -m uvicorn api.main:app --host 0.0.0.0 --port 8000
```

### Вариант 2: Запуск через systemd

```bash
# Скопировать unit файл
sudo cp scripts/veil-xray-api.service /etc/systemd/system/

# Перезагрузить systemd
sudo systemctl daemon-reload

# Запустить сервис
sudo systemctl start veil-xray-api

# Включить автозапуск
sudo systemctl enable veil-xray-api

# Проверить статус
sudo systemctl status veil-xray-api
```

## 🔧 Настройка Xray

1. Скопируйте приватный ключ из `/root/config/reality_keys.txt`
2. Добавьте его в конфигурацию Xray (`/usr/local/etc/xray/config.json`) в секцию `realitySettings.privateKey`
3. Убедитесь, что Xray API включен в конфигурации:

```json
{
  "api": {
    "tag": "api",
    "services": ["StatsService", "HandlerService"]
  }
}
```

4. Перезапустите Xray:

```bash
sudo systemctl restart xray
```

## 📝 Проверка работы

После запуска API сервера:

1. Проверьте документацию API: http://localhost:8000/docs
2. Проверьте здоровье сервиса: http://localhost:8000/
3. Создайте тестовый ключ:

```bash
curl -X POST "http://localhost:8000/api/keys" \
  -H "Authorization: Bearer $(grep API_SECRET_KEY /root/.env | cut -d'=' -f2)" \
  -H "Content-Type: application/json" \
  -d '{"name": "test_key"}'
```

## 🔐 Безопасность

⚠️ **ВАЖНО:**
- Измените `API_SECRET_KEY` в `.env` на более сложный ключ для production
- Храните приватный ключ Reality в безопасном месте
- Настройте firewall для ограничения доступа к API
- Используйте HTTPS через reverse proxy (Nginx/Caddy)

## 📚 Дополнительная информация

- Полная документация: `README.md`
- Техническое задание: `TZ.md`
- Конфигурация: `.env`



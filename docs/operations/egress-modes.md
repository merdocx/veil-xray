...
**По умолчанию** достаточно outbound **`direct`** (трафик выходит с IP самого veil-xray). SOCKS и WireGuard — **опциональные** варианты egress (**B/C**), см. [topologies.md](topologies.md).

**Front relay (E)** — релей **перед** VPN (L4 TCP), не egress: [relay-front-topology.md](relay-front-topology.md).
...
В шаблонах репозитория используется плейсхолдер `<RELAY_HOST>` / `<RELAY_IP>` — подставьте IP **вашего** egress-релея (страна не фиксируется).

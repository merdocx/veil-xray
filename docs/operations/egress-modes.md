...
**По умолчанию** достаточно outbound **`direct`** (трафик выходит с IP самого veil-xray). SOCKS и WireGuard — **опциональные** варианты, см. [ru-bridge-chain.md](ru-bridge-chain.md) (таблица A–D). RU-мост — отдельная опция D, не часть egress.

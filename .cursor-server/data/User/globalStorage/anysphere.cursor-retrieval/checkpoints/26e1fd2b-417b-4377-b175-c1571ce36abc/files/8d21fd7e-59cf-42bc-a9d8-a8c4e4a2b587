...
- **Short IDs:** Генерируются автоматически для каждого ключа
...
- **Short ID** (для Reality): Уникальный короткий идентификатор для Reality
  - Генерируется автоматически для каждого ключа
  - Используется для маршрутизации трафика в Reality
  - Может храниться в базе данных или генерироваться на основе UUID

...
- **SNI:** veil-bear.ru (фиксированный для всех ключей)
- **Fingerprint (FP):** Фиксированный TLS fingerprint (например, "chrome", "firefox", "safari")
- **Short IDs:** Генерируются автоматически для каждого ключа (8 символов, хранятся в базе данных)
- **Dest:** Настроенный сайт-приманка
...
**Reality параметры:**
- **Server Name (SNI):** veil-bear.ru (фиксированный для всех ключей)
- **Fingerprint (FP):** Фиксированный TLS fingerprint для имитации клиента (например, "chrome", "firefox", "safari" и т.д.)
...
- **Dest:** Адрес сайта-приманки (например, www.microsoft.com:443)
- **Server Names:** Список разрешенных SNI (обычно включает veil-bear.ru)

...
- **Параметры запроса:**
  ...
  - `expires_at` (опционально): Дата истечения ключа (timestamp)
- **Ответ:**
...
- **Ответ:**
  ...
  - `qr_code` (опционально): Base64 изображение QR-кода для мобильных клиентов
- **Требования:**
...
- `GET /api/stats` - Общая статистика сервера
...
- `expires_at` (INTEGER, NULLABLE): Timestamp истечения
...
```
/
├── api/              # API сервер (Python/Go/Node.js)

...
**Мульти-страновую** подписку (NL, DE, KZ, …) собирает **управляющий сервер** (veilbot): пользователь импортирует **одну** ссылку бота, внутри неё — несколько серверов; один из пунктов — конфиг с этой панели.
...
| **2** | Подписка с автовыбором (внутри общей подписки бота) | Одна URL подписки от бота; в списке стран — этот узел; автопереключение портов/SNI **внутри** узла | `GET /api/keys/{id}/bot-bundle` или `GET .../subscription?profiles=auto&format=singbox_b64` |
...
- в списке **один** профиль (например, «Казахстан» из `remark`);
...
В приложении — список **стран / серверов** (NL, DE, KZ, …). Их формирует **veilbot**, не veil-xray.

```text
[Пользователь]
      │
      ▼
  Одна URL подписки veilbot  ──►  Happ:  NL │ DE │ KZ │ …
                                      │
                                      └── «KZ» = конфиг с панели veil-xray
```

#### Уровень 2 — конфиг с панели (внутри пункта «этот сервер»)

Для пункта «KZ» (или как назван сервер в боте) veilbot при создании ключа запрашивает панель:
...
Пользователь в Happ для этой страны видит **одну** запись; переключение 448 ↔ 447 — автоматически, без второй подписки в UI.

**Автовыбор между разными VPS (странами)** — только на стороне **veilbot** (несколько панелей или прокси в одной подписке). Veil-xray отдаёт конфиг **одного** хоста.

#### Рекомендуемый поток для разработчика veilbot

1. `POST /api/keys` на панели KZ (и при необходимости на других панелях).
2. Для KZ сохранить `subscription_singbox_b64` из `bot-bundle` (или URL прокси на `.../subscription?profiles=auto&format=singbox_b64`).
3. В **общей** подписке пользователя добавить профиль «KZ» с этим конфигом.
...
| **«Подписка»** | Выдать URL veilbot с `format=happ`; внутри уже есть sing-box с панели | Одна подписка, много стран; на KZ — urltest 448/447 при SNI-B |
...
| Прямая выдача пользователю URL панели | Обходит мульти-страновую логику veilbot; допустимо только для отладки |

...
- [UX для пользователя и интеграция с veilbot](#ux-для-пользователя-и-интеграция-с-veilbot)
- [Endpoints](#endpoints)
...
## UX для пользователя и интеграция с veilbot

Veil-xray — **панель одного входного узла**: выдаёт ключи и конфиги для **этого** сервера.  
**Мульти-серверную** подписку (`<server-a>`, `<server-b>`, …) собирает **управляющий сервер** (veilbot): пользователь импортирует **одну** ссылку бота, внутри неё — несколько узлов; один из пунктов — конфиг с этой панели.

Рекомендуемые **два варианта** для конечного пользователя:

| # | Вариант | Что видит пользователь | Что запрашивает бот у панели |
|---|---------|------------------------|------------------------------|
| **1** | Обычная VLESS | Одна ссылка или QR, один сервер в клиенте | `GET /api/keys/{id}/link` |
| **2** | Подписка с автовыбором (внутри общей подписки бота) | Одна URL подписки от бота; в списке серверов — этот узел; автопереключение портов/SNI **внутри** узла | `GET /api/keys/{id}/bot-bundle` или `GET .../subscription?profiles=auto&format=singbox_b64` |

Эксплуатация панели: [docs/OPERATIONS.md](docs/OPERATIONS.md).

---

### Вариант 1 — обычная VLESS-ссылка

**Поток:** пользователь в Telegram → бот создаёт ключ → отдаёт **одну** строку `vless://…` (или QR).

**В VPN-клиенте (v2rayNG, Happ, Shadowrocket):**
- в списке **один** профиль (имя из `remark` в ссылке, задаётся панелью);
- подключение к `DOMAIN` из `.env` панели (IP или DNS входного узла / front relay, см. [relay-front-topology.md](docs/operations/relay-front-topology.md)).

**Параметр `profile` у `/link`:**

| `profile` | Порт / транспорт | Кому подходит |
|-----------|------------------|---------------|
| `primary` (по умолчанию) | TCP **443**, Vision (`xtls-rprx-vision`) | Классические Xray-клиенты |
| `happ`, `auto` | TCP **448**, без Vision | Happ / iOS |

**Пример для бота (выдача ссылки в чат):**

```bash
curl -sS -H "Authorization: Bearer $API_SECRET_KEY" \
  "https://api.example.com/api/keys/42/link?profile=primary"
```

Ответ: `{ "key_id": 42, "vless_link": "vless://..." }` — эту строку копирует пользователь.

Пользователь **не** получает автоматически порты 446, 447, XHTTP, Trojan, если бот не вызывает `/links` или `/subscription?profiles=all`.

---

### Вариант 2 — подписка с автовыбором (вложенная в подписку veilbot)

Здесь **два уровня**: внешняя подписка бота и внутренний конфиг панели.

#### Уровень 1 — подписка veilbot (видит пользователь)

Пользователь **один раз** добавляет в Happ ссылку **бота**, например:

```text
https://<veilbot>/api/subscription/<user_token>?format=happ
```

В приложении — список **серверов** (`<server-a>`, `<server-b>`, …). Их формирует **veilbot**, не veil-xray.

```text
[Пользователь]
      │
      ▼
  Одна URL подписки veilbot  ──►  Happ:  <server-a> │ <server-b> │ …
                                      │
                                      └── <server-b> = конфиг с панели veil-xray
```

#### Уровень 2 — конфиг с панели (внутри пункта «этот сервер»)

Для пункта `<server-b>` (как назван узел в боте) veilbot при создании ключа запрашивает панель:

```bash
# Сводка для бота (рекомендуется после POST /api/keys)
curl -sS -H "Authorization: Bearer $API_SECRET_KEY" \
  "https://api.example.com/api/keys/42/bot-bundle"

# Или эквивалент:
curl -sS -H "Authorization: Bearer $API_SECRET_KEY" \
  "https://api.example.com/api/keys/42/subscription?profiles=auto&format=singbox_b64"
```

| Поле / ответ | Назначение |
|--------------|------------|
| `bot-bundle.vless_happ` | Одна `vless://` на **:448** (запасной ручной импорт) |
| `bot-bundle.subscription_singbox_b64` | base64 sing-box JSON для встраивания в подписку бота |
| `bot-bundle.singbox` | Тот же JSON открытым текстом |

**Почему не несколько `vless://` в `profiles=auto`:** при `format=base64` / `plain` панель отдаёт **одну** ссылку на порт **448**. Несколько строк в подписке ломают Happ (observatory с пустым DNS). Автовыбор делается **внутри sing-box JSON**, не списком ссылок.

**Автовыбор на одном узле (внутри sing-box):**

| Настройка панели | Поведение в клиенте |
|------------------|---------------------|
| Только основной SNI | один outbound → **448** |
| Включён SNI-B (`REALITY_SNI_B`, ключи B) | outbound **448** + **447** + блок **`urltest`** — клиент пингует и выбирает лучший **на том же `DOMAIN`** |

Пользователь в Happ для этой записи видит **одну** строку; переключение 448 ↔ 447 — автоматически, без второй подписки в UI.

**Автовыбор между разными VPS** — только на стороне **veilbot** (управляющий сервер): в одной подписке бота несколько `<server-a>`, `<server-b>`, клиент или бот выбирает узел.

**Автовыбор на этом сервере (veil-xray)** — в sing-box JSON (`profiles=auto`, `format=singbox_b64`): блок **`urltest`** между портами **448** и **447** (если включён SNI-B). Это **не** veilbot; панель отдаёт готовый конфиг для одного хоста.

#### Рекомендуемый поток для разработчика veilbot

1. `POST /api/keys` на панели `<server-b>` (и при необходимости на других панелях).
2. Для `<server-b>` сохранить `subscription_singbox_b64` из `bot-bundle` (или URL прокси на `.../subscription?profiles=auto&format=singbox_b64`).
3. В **общей** подписке пользователя добавить профиль `<server-b>` с этим конфигом.
4. Пользователю выдать **только** URL подписки veilbot (`?format=happ`), не URL панели напрямую.
5. После `POST /api/keys` **не** вызывать `POST /api/system/xray/sync-config` (ключ уже в Xray через hot-add).

#### Две кнопки в боте (пример UX)

| Кнопка | Действие бота | Ощущение пользователя |
|--------|---------------|------------------------|
| **«Ссылка»** | `/link?profile=primary` (или `happ`) → отправить `vless_link` | Один сервер, ручное подключение |
| **«Подписка»** | Выдать URL veilbot с `format=happ`; внутри уже есть sing-box с панели | Одна подписка, много серверов; на этом узле — urltest 448/447 при SNI-B |

---

### Что не использовать для варианта 2 (Happ)

| Endpoint | Почему |
|----------|--------|
| `GET .../subscription?profiles=auto&format=base64` с несколькими ссылками | В `auto` — **одна** ссылка :448; мульти-ссылки — только `profiles=all` (не для Happ) |
| `GET .../client-config` | Xray observatory + leastPing; для Happ `auto` — **sing-box** (`singbox_b64`), не этот JSON |
| Прямая выдача пользователю URL панели | Обходит мульти-серверную логику veilbot; допустимо только для отладки |

---

### Сводка ответственности

| Компонент | Зона ответственности |
|-----------|----------------------|
| **veil-xray** | UUID, hot-add в Xray, VLESS/sing-box для **одного** входного узла, трафик, `DOMAIN` в ссылках |
| **veilbot** | Оплата, **одна** пользовательская подписка, список серверов, вложение `subscription_singbox_b64` с каждой панели |
| **Клиент (Happ)** | Импорт подписки бота; urltest 448/447 внутри профиля одного узла (если SNI-B) |

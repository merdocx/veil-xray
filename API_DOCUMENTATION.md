...
| `ru` | XHTTP **8445** | Только при топологии RU-мост ([ru-bridge-chain.md](docs/operations/ru-bridge-chain.md)) |
...
**Автовыбор между разными VPS** — только на стороне **veilbot** (несколько панелей или прокси в одной подписке). Veil-xray отдаёт конфиг **одного** хоста.
...
| **veilbot** | Оплата, **одна** пользовательская подписка, список стран, вложение `subscription_singbox_b64` с каждой панели |
| **Клиент (Happ)** | Импорт подписки бота; urltest внутри sing-box профиля страны |

...
class XrayClient:
    ...
    async def _add_user_internal(self, uuid: str, email: str, flow: str = "none") -> bool:
        """
        ...
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            ...
            if response.status_code == 200:
                logger.info(f"✅ User {uuid[:8]}... (email: {email}) added successfully to Xray via API")
                return True
            else:
    ...
    @retry(
        ...
        ) if retry_state.attempt_number < 3 else None
    )
